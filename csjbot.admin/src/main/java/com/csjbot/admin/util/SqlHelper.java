...
/*    public static String getMapperSql(Object mapper, String methodName, Object... args) {
    ...
    //单个对象参数
    public static String getBeanSql(SqlSession session, String fullMapperMethodName, Object obj) {
        String methodName = fullMapperMethodName.substring(fullMapperMethodName.lastIndexOf('.') + 1);
        Class<?> mapperInterface = null;
        try {
            mapperInterface = Class.forName(fullMapperMethodName.substring(0, fullMapperMethodName.lastIndexOf('.')));
            return getBeanSql(session, mapperInterface, methodName, obj);
        } catch (ClassNotFoundException e) {
        	e.printStackTrace();
            throw new IllegalArgumentException("参数" + fullMapperMethodName + "无效！");
        }
    ...
    public static String getBeanSql(SqlSession session, Class<?> mapperInterface, String methodName, Object obj) {
        String fullMapperMethodName = mapperInterface.getCanonicalName() + "." + methodName;
        Map<String, Object> map = new HashMap<String, Object>(); 
        map = MapUtil.objToMap(obj);
        return getBeanObjectSql(session, fullMapperMethodName, map);
    }
    
    public static String getBeanObjectSql(SqlSession session, String namespace, Object params) {
        Configuration configuration = session.getConfiguration();
        MappedStatement mappedStatement = configuration.getMappedStatement(namespace);
        TypeHandlerRegistry typeHandlerRegistry = mappedStatement.getConfiguration().getTypeHandlerRegistry();
        BoundSql boundSql = mappedStatement.getBoundSql(params);
